...
export const UPDATE_EES_DATA = async (id, data) => {
	...
	console.log('UPDATE_EES_DATA', type, count_type, symbol, percent, percent_range, description);
...
export const UPDATE_EMPLOYEE_JUVENILE_STATUS = async (id, data) => {
	...
	return await employeesDB.asyncUpdate({ _id: id}, { $set: { 'juvenile_worker': data.newJuvenileStatus}})
}

export const UPDATE_EMPLOYEE_TERMINATION_DATE = async (id, data) => {
	// console.log('UPDATE_EMPLOYEE_TERMINATION_DATE', id, data);
	return await employeesDB.asyncUpdate({ _id: id}, { $set: { 'employment_termination_date': data.date, 'employment_status': data.status }})
}
...
export const UPDATE_EMPLOYEE_HOLIDAY_DATES = async (id, queryFields, data) => {
	...
	return await employeesDB.asyncUpdate({ _id: id }, { $push: { [`calendar.${employeeYearIndex}.months.${employeeMonthIndex}.holiday_leave`]: data.date }})
}
...
export const REMOVE_EMPLOYEE_HOLIDAY_DATES = async (id, queryFields, data) => {
	...
	const employeeYearIndex = employee.calendar.findIndex(year => year.year === queryFields.year)
	const employeeMonthIndex = employee.calendar.filter(year => year.year === queryFields.year)[0].months.findIndex(el => el.month === queryFields.month)
	return await employeesDB.asyncUpdate({ _id: id }, { $pull: { [`calendar.${employeeYearIndex}.months.${employeeMonthIndex}.holiday_leave`]: data }})
}
...
export const UPDATE_EMPLOYEE_SICK_DATES = async (id, queryFields, data) => {
	...
	const employeeYearIndex = employee.calendar.findIndex(year => year.year === queryFields.year)
	const employeeMonthIndex = employee.calendar.filter(year => year.year === queryFields.year)[0].months.findIndex(el => el.month === queryFields.month)
	return await employeesDB.asyncUpdate({ _id: id }, { $push: { [`calendar.${employeeYearIndex}.months.${employeeMonthIndex}.sick_leave`]: data.date }})
}
...
export const REMOVE_EMPLOYEE_SICK_DATES = async (id, queryFields, data) => {
	...
	const employeeYearIndex = employee.calendar.findIndex(year => year.year === queryFields.year)
	const employeeMonthIndex = employee.calendar.filter(year => year.year === queryFields.year)[0].months.findIndex(el => el.month === queryFields.month)
	return await employeesDB.asyncUpdate({ _id: id }, { $pull: { [`calendar.${employeeYearIndex}.months.${employeeMonthIndex}.sick_leave`]: data }})
}
...
export const UPDATE_EMPLOYEE_OTHER_LEAVE_DATES = async (id, queryFields, data) => {
	...
	const employeeYearIndex = employee.calendar.findIndex(year => year.year === queryFields.year)
	const employeeMonthIndex = employee.calendar.filter(year => year.year === queryFields.year)[0].months.findIndex(el => el.month === queryFields.month)
	return await employeesDB.asyncUpdate({ _id: id }, { $push: { [`calendar.${employeeYearIndex}.months.${employeeMonthIndex}.other_leave`]: data.date }})
}
...
export const REMOVE_EMPLOYEE_OTHER_LEAVE_DATES = async (id, queryFields, data) => {
	...
	const employeeYearIndex = employee.calendar.findIndex(year => year.year === queryFields.year)
	const employeeMonthIndex = employee.calendar.filter(year => year.year === queryFields.year)[0].months.findIndex(el => el.month === queryFields.month)
	return await employeesDB.asyncUpdate({ _id: id }, { $pull: { [`calendar.${employeeYearIndex}.months.${employeeMonthIndex}.other_leave`]: data }})
}
...
export const UPDATE_EMPLOYEE_RTS_DATA = async (id, queryFields, data) => {
	...
	const rtsDayIndex = employee.calendar.filter(year => year.year === queryFields.year)[0].months.filter(el => el.month === queryFields.month)[0].rts.findIndex(day => JSON.stringify(day.due_date) === JSON.stringify(data.due_date))
	...
	return await employeesDB.asyncUpdate({ _id: id }, { $set: { [`calendar.${employeeYearIndex}.months.${employeeMonthIndex}.rts.${rtsDayIndex}`]: data }})
...
export const UPDATE_EMPLOYEE_MONTHLY_RATES = async (id, data) => {
  ...
  const resArray = []
	for ( let monthIterator = month; monthIterator <= 12; monthIterator++ ) {
		const employeeMonthIndex = employee.calendar.filter(y => y.year === year)[0].months.findIndex(el => el.month === monthIterator)
		// console.log('employeeYearIndex', employeeYearIndex, 'employeeMonthIndex', employeeMonthIndex)
		const res = await employeesDB.asyncUpdate({ _id: id }, { $set: {
