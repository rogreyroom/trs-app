...
const getEmployees = async () => await axios.get('/api/employees')
...
const Employees = () => {
  ...
  employees.then(res => {
    ...
    if (!hasCurrentYear) {
      const previousYearMonthsData = employeeCalendar.filter(({ year }) => year === currentYear - 1)[0].months
      const previousYearLastMonthData = previousYearMonthsData[previousYearMonthsData.length - 1]
      const newData = createCalendarForCurrentYear(currentYear, previousYearLastMonthData)
      newData.then(monthsData => {
        axios.put(`/api/employees/${employee._id}`, { field: 'newYear', value: { ...monthsData }})
