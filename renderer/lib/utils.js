...
export const getEmployeeYears = (employeeCalendar) => {
  return employeeCalendar.map(({year}) => parseInt(year));
};

export const getUsedHolidayDays = (employeeCalendar) => {
  const currYear = new Date().getFullYear();
  const currentYearMonths = employeeCalendar.filter(
    ({year}) => parseInt(year) === currYear,
  )[0]?.months;
...
export const getGivenMonthData = (employeeCalendar, year, month) => {
  ...
  return givenYearMonths?.filter(({month}) => parseInt(month) == givenMonth);
};
...
export const getCurrentMonthWorkedHours = (currentMonthData) => {
  const workingHoursArray = currentMonthData.rts.map(
...
export const getCurrentMonthOvertimeHours = (currentMonthData) => {
  const overtimeHoursArray = currentMonthData.rts.map(
...
export const getCurrentMonthWeekendsHours = (currentMonthData) => {
  const weekendHoursArray = currentMonthData.rts.map(
