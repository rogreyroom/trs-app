...
export const LeaveForm = ({ id }) => {
  ...
  const onSubmit = async (data) => {
    ...
    if ( datesRange.from.year === datesRange.to.year && datesRange.from.month === datesRange.to.month ) {
      ...
      await axios.put(`/api/employees/${id}`, { field: 'other', queryFields: { year: yearIs, month: monthIs }, value: { date: valueIs }})
    }
    ...
    if ( datesRange.from.year === datesRange.to.year && datesRange.from.month !== datesRange.to.month ) {
      ...
      await axios.put(`/api/employees/${id}`, { field: 'other', queryFields: { year: yearIs, month: firstMonthIs }, value: { date: firstValueIs }})
