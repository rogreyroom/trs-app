...
const schema = Joi.object().keys({
  sickRangeDates: Joi.object().keys({
...
export const SickForm = ({ id }) => {
  ...
  const onSubmit = async (data) => {
    ...
    if ( datesRange.from.year === datesRange.to.year && datesRange.from.month === datesRange.to.month ) {
      ...
      await axios.put(`/api/employees/${id}`, { field: 'sick', queryFields: { year: yearIs, month: monthIs }, value: { date: valueIs }})
    }
    ...
    if ( datesRange.from.year === datesRange.to.year && datesRange.from.month !== datesRange.to.month ) {
      ...
      await axios.put(`/api/employees/${id}`, { field: 'sick', queryFields: { year: yearIs, month: firstMonthIs }, value: { date: firstValueIs }})
      await axios.put(`/api/employees/${id}`, { field: 'sick', queryFields: { year: yearIs, month: secondMonthIs }, value: { date: secondValueIs }})
    }
