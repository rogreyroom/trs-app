...
export const EmployeeCalendar = forwardRef(({employeeData}, ref) => {
  ...
  const getWorkingDay = (day, dayObject, data) => {
    ...
    const returnData =
      isSaturday(day) || isSunday(day)
        ...
        : (hoursData && {
            hours: hoursData.working_hours,
            overtime: hoursData.overtime_hours,
          }) || {hours: 0, overtime: 0};
    return returnData;
  };
  ...
  return (
    <PrintArea ref={ref}>
      ...
      <div>
        <p>
          Ilość godzin: <span>{workedHours}</span> <span>godz</span>
        </p>
        <p>
          Ilość nadgodzin: <span>{overtimeHours}</span> <span>godz</span>
        </p>
        <p>
          Ilość dyżurów: <span>{weekendsHours}</span> <span>godz</span>
        </p>
        <p>
          Ilość urlopu: <span>{holidayDays}</span>{' '}
          <span>{holidayDays === 1 ? 'dzień' : 'dni'}</span>
        </p>
        <p>
          Ilość urlopu okolicznościowego: <span>{sickDays}</span>{' '}
          <span>{sickDays === 1 ? 'dzień' : 'dni'}</span>
        </p>
        <p>
          Ilość chorobowego: <span>{otherLeaveDays}</span>{' '}
