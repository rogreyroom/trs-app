...
const EmployeeRcpDetails = ({year, month}) => {
  ...
  const amountToBePaid = ( amountSumOfHours + amountSumOfLeave ) - toAccountRate - insuranceRate - retainmentRate
      ...
      <StyledPrintArea ref={tableRef}>
        ...
        <table>
          ...
          <tbody>
            <tr>
              <td>Obecność</td>
              <td>{ workedHours } <span>godz</span></td>
              <td>{ amountWorkedHours } <span>pln</span></td>
            </tr>
            ...
            <tr>
              <td>Urlop</td>
              <td>{ holidayDays } <span>{holidayDays === 1 ? 'dzień' : 'dni' }</span></td>
              <td>{ amountHolidayDays } <span>pln</span></td>
            </tr>

            <tr>
              <td>Chorobowe</td>
              <td>{ sickDays }  <span>{sickDays === 1 ? 'dzień' : 'dni' }</span></td>
              <td>{ amountSickDays } <span>pln</span></td>
            </tr>

            <tr>
              <td>Urlop okolicznościowy</td>
              <td>{ otherLeaveDays }  <span>{otherLeaveDays === 1 ? 'dzień' : 'dni' }</span></td>
              <td>{ amountOtherLeaveDays } <span>pln</span></td>
            </tr>

            <tr className='tableSumRow'>
              <td>SUMA</td>
              <td>{ holidayDays + sickDays + otherLeaveDays }  <span>{(holidayDays + sickDays + otherLeaveDays) === 1 ? 'dzień' : 'dni' }</span></td>
              <td>{ amountSumOfLeave } <span>pln</span></td>
            </tr>

            <tr>
              <td>ROR</td>
              <td>-</td>
              <td>{ toAccountRate } <span>pln</span></td>
            </tr>

            <tr>
              <td>PZU</td>
              <td>-</td>
              <td>{ insuranceRate } <span>pln</span></td>
            </tr>

            <tr>
              <td>PPK</td>
              <td>-</td>
              <td>{ retainmentRate } <span>pln</span></td>
            </tr>
                ...
                <div>
                  ...
                  { autoWeekdayBonus > 0 &&  <p>{ autoWeekdayBonusName }</p> }
                  {
                    ...
                    evaluationArray.map(({name, percent, description}, idx) => {
                      return (
                        <p key={idx}>{ name } { percent } %, { description }</p>
                      )
                    })
                  }
                </div>
              </td>
              <td>{ amountBonus } <span>pln</span></td>
            </tr>
